...
fn run_once_blocking(events: u64, shard_count: usize) -> BenchmarkResult {
    ...
    let sharded = Arc::try_unwrap(sharded)
        .unwrap_or_else(|_| panic!("Failed to unwrap Arc<ShardedPipeline> - references still exist"));
    sharded.stop();

    // Use max thread duration as the effective duration (bottleneck)
    BenchmarkResult::new(
        &format!("Sharded Pipeline ({} shards, {} producers)", shard_count, num_producers),
        total_events,
        max_duration,
        &mut vec![], // No per-event latency tracking in multi-threaded mode
    )
}
