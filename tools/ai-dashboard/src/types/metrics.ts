...
export interface EventMessage {
  ...
  kind:
    ...
    | "info";
  message: string;
  fault?: string;  // present for fault_inject / fault_recover events
}
...
export interface DataloaderSummaryMessage {
  ...
  bandwidth_gb_s: number;  // Transport bandwidth (GB/s)
  total_gb: number;        // Total data transferred (GB)
  p50_us: number;
  p99_us: number;
  p999_us: number;
  wall_secs: number;
...
export interface InferenceSummaryMessage {
  ...
  samples_per_sec: number;
  batches_per_sec: number;
  bandwidth_gb_s: number;  // Input bandwidth (GB/s)
  total_gb: number;        // Total input data (GB)
  p50_us: number;
  p99_us: number;
  p999_us: number;
  wall_secs: number;
