    ...
    >
      ...
      <div className="flex items-center gap-3 min-w-0">
        ...
        {config && (
          ...
          >
            {'mode' in config && <span>{config.mode}</span>}
            {'mode' in config && <span style={{ color: "var(--border-bright)" }}>·</span>}
            <span>
              {config.duration_secs ? `${config.duration_secs}s` : "event mode"}
            </span>
            {'dataset' in config && <span style={{ color: "var(--border-bright)" }}>·</span>}
            {'dataset' in config && (
              <span className="truncate max-w-[120px]" title={config.dataset}>
                {config.dataset}
              </span>
            )}
            {'batch_size' in config && <span style={{ color: "var(--border-bright)" }}>·</span>}
            {'batch_size' in config && <span>batch={String(config.batch_size)}</span>}
            {'num_workers' in config && <span style={{ color: "var(--border-bright)" }}>·</span>}
            {'num_workers' in config && <span>workers={String(config.num_workers)}</span>}
          </div>
        )}
      </div>
