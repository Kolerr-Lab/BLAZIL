...
extern "C" {
    #[doc = " Poll for async add publication completion.\n\n @param publication set when complete.\n @param async handle returned by aeron_async_add_publication.\n @return 1 if complete, 0 if not yet, -1 on error."]
...
extern "C" {
    #[doc = " Poll for async add subscription completion.\n\n @param subscription set when complete.\n @param async handle returned by aeron_async_add_subscription.\n @return 1 if complete, 0 if not yet, -1 on error."]
...
// ── Embedded Media Driver bindings ────────────────────────────────────────────
// These opaque types and functions are part of `libaeron_driver_static`.
// They are absent from the client-only Aeron archive but required by
// `blazil-transport`'s `EmbeddedAeronDriver` when the `aeron` feature is
// enabled.  On macOS / CI without the C library these stubs allow
// `cargo check` and `cargo clippy` to type-check driver.rs; linking will
// only succeed on Linux where `build.rs` compiles `libaeron_driver_static.a`.
