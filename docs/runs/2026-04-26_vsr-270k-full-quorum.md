...
**269,600 TPS** represents the baseline performance of a **fully healthy 3-node VSR cluster** with optimal network conditions and all nodes responding within the same datacenter.
...
|---------------|-----|-------|
| **Full quorum (this run)** | **269,600** | All nodes healthy, optimal conditions |
| **1 node killed** | 233,894 | Still 2/3 quorum, degraded perf |
| **3-shard aggregate** | 436K | No consensus, pure sharding |
| **Single-node (no VSR)** | 130-160K | Individual node throughput |

### Bottleneck

At 269,600 TPS, the bottleneck is **batch throughput vs end-to-end latency tradeoff**:
```
Batch throughput = Batch_size / VSR_batch_time
                 = 8,190 transfers / 0.030s
                 = 273,000 TPS theoretical
                 ≈ 269,600 TPS measured (99% efficiency)

End-to-end latency = P50 ~278ms, P99 ~312ms
  = Ring buffer queueing + batch formation + VSR consensus + result propagation
```

The system achieves **high throughput** (269,600 TPS) at the cost of **higher latency** (~300ms) due to batching strategy.
...
✅ **Performance:** 269,600 TPS exceeds most production requirements  
✅ **Latency:** P99 ~312ms suitable for high-throughput batch processing  

**Verdict:** Production-ready configuration. 269,600 TPS is the proven throughput for a fully healthy VSR cluster.

---

## Related Benchmarks

- [VSR with 1 node killed (233,894 TPS)](./2026-04-26_vsr-233k-node-killed.md)
